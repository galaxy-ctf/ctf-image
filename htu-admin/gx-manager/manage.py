#!/usr/bin/env python
"""
Script for managing launching galaxy instances to match those contained in a TSV file.
"""
import re
import uuid
import requests
...
RANCHER_ACCESS_KEY="xxxxxxxxxxxxxxxxxxxx"
RANCHER_SECRET_KEY="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
INSTANCE_DATA = [
    {'team_id': y[0], 'team_name': y[1], 'password': y[2]} for y in
    [x.split('\t') for x in open('instances.tsv', 'r').read().strip().split('\n')]
]
METADATA_FLAG = "gccctf{xxxxxxxxxxxxxxxxxxxxxx}"

class FakeTeam(object):
    def __init__(self, id, name, password, admin_password):
        self.id = id
        self.name = name
...
def sync_state():
    ...
    stateChanged = False
    if True:
        for TEAM_INSTANCE in INSTANCE_DATA:
            safe_team_name = safe_str(TEAM_INSTANCE['team_name'])
            safe_team_id = int(TEAM_INSTANCE['team_id'])
            safe_team_password = safe_str(TEAM_INSTANCE['password'])

            team = FakeTeam(safe_team_id, safe_team_name, safe_team_password, 'galaxians')
